...
//   MotionPlan (S-curve) → SyncExecutive @500 Hz → Governor → adapter
//
...
int main(int argc, char** argv) {
    ...
    robonode::SyncExecutive exec{{wrist.get()}, {&governor}, /*rate_hz=*/500.0, {},
                                 /*stop_time_s=*/0.3};
    ...
    const auto stats = exec.execute({&source}, all, plan.duration_s() + 0.3);
