...
void test_arm_seven_dof_one_clock() {
    ...
    CHECK(cell.run_waypoints(kWaypoints, 1000.0, rows, stats, /*settle_s=*/1.0,
                                 /*stop_time_s=*/0.3).ok());
...
void test_arm_fault_on_one_joint_holds_all() {
    ...
    robonode::SyncExecutive exec{adapters, gp, 1000.0, {}, 0.3};

    const auto plan = *robonode::SyncBlendPlan::make(
