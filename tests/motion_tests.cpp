...
void test_executive_completes_scurve_move() {
    ...
    const auto stats = testing::run_plan(axis, gov, 1000.0, plan, rows, /*settle_s=*/0.1);
...
// On an IDEAL axis — the plant answers the setpoint instantly, so what is left
// is the command itself. Smoothing wins here and the number says so.
//
// It does NOT win on the real cell: under inertia a smoothing controller lags
// more through the acceleration phase, so its PEAK following error is worse.
// Both are true, and `robonode compare control robonode.direct robonode.smooth`
// reports the second one. See test_smoothing_costs_peak_error_on_a_real_plant.
void test_control_smooth_tracks_gentler_than_direct_on_an_ideal_axis() {
    const auto run = [](robonode::Controller* ctrl, double& peak_err, double& final_pos) {
        robonode::SimAxis axis{"t", 0.0, 0.02};
        robonode::Governor gov{kLimits};
        std::vector<robonode::AxisAdapter*> ad{&axis};
        std::vector<robonode::Governor*> gv{&gov};
        std::vector<robonode::Controller*> ct;
        if (ctrl) ct.push_back(ctrl);
        robonode::SyncExecutive exec{ad, gv, 1000.0, ct, 0.3};
        const auto plan = *robonode::SyncBlendPlan::make({{0.0, 500.0}}, {kLimits});
        std::vector<std::vector<robonode::TelemetryRow>> rows;
        robonode::CycleStats stats{};
        stats = exec.execute(plan, rows, /*settle_s=*/0.3);
        peak_err = 0.0;
        for (const auto& r : rows[0]) peak_err = std::max(peak_err, std::abs(r.following_error));
        final_pos = rows[0].back().actual_position;
    };
    robonode::DirectController direct;
    double direct_err = 0.0, direct_pos = 0.0;
    run(&direct, direct_err, direct_pos);

    robonode::SmoothController smooth{0.08};
    double smooth_err = 0.0, smooth_pos = 0.0;
    run(&smooth, smooth_err, smooth_pos);

    CHECK(std::abs(direct_pos - 500.0) < 1.0);  // both converge to the same target
    CHECK(std::abs(smooth_pos - 500.0) < 1.0);
    CHECK(smooth_err < direct_err);  // on an ideal plant, smoothing is strictly gentler
}
...
void test_executive_holds_on_protective_stop_and_recovers() {
    ...
    std::vector<robonode::TelemetryRow> rows;
    const auto plan = robonode::MotionPlan::move(0.0, 500.0, kScurveProfile);
    const auto stats = testing::run_plan(axis, gov, 1000.0, plan, rows, /*settle_s=*/0.5);
...
void test_sync_plan_passes_through_monotonic_via() {
    ...
    const auto plan = *robonode::SyncBlendPlan::make(wp, {kLimits, kAuxLimits});
...
void test_executive_streams_retargeted_otg_through_governor() {
    ...
    std::vector<robonode::TelemetryRow> rows;
    const auto stats = testing::run_source(axis, gov, 1000.0, stream, rows, /*run_for_s=*/1.6);
...
void test_sync_executive_two_axes_settle_together() {
    ...
    robonode::SyncExecutive exec{{&rail, &aux}, {&g0, &g1}, 1000.0, {}, 0.3};

    const auto plan = *robonode::SyncBlendPlan::make({{0.0, 500.0, 300.0}, {0.0, 90.0, 45.0}},
                                                    {kLimits, kAuxLimits});
    std::vector<std::vector<robonode::TelemetryRow>> rows;
...
void test_sync_executive_coherent_hold() {
    ...
    robonode::SyncExecutive exec{{&normal_axis, &faulting}, {&g0, &g1}, 1000.0, {}, 0.3};

    const auto plan = *robonode::SyncBlendPlan::make({{0.0, 500.0}, {0.0, 500.0}}, {kLimits, kLimits});
    std::vector<std::vector<robonode::TelemetryRow>> rows;
...
// A stop request decelerates ALONG the plan (stop category 2): the run ends
// early, reports it, and the axis never steps to a hold.
void test_cancel_decelerates_on_path_and_reports() {
    robonode::SimAxis axis{"rail", 0.0, 0.005};
    robonode::Governor gov{kLimits};
    const auto plan = *robonode::SyncBlendPlan::make({{0.0, 900.0}}, {kLimits});
    robonode::CancelToken cancel;
    robonode::SyncExecutive exec{{&axis}, {&gov}, 1000.0, {}, 0.3};
    ...
    std::vector<std::vector<robonode::TelemetryRow>> rows;
    const auto stats = exec.execute(plan, rows, /*settle_s=*/0.05, {}, &cancel);
    stopper.join();

    CHECK(stats.stopped_early);
    CHECK(stats.cancel_cycles > 0);
    CHECK(rows[0].back().actual_position < 900.0);  // never reached the goal
    // The commanded setpoint slows to a stop instead of jumping.
    const auto& r = rows[0];
    const double last_step = std::abs(r.back().governed_position - r[r.size() - 2].governed_position);
    CHECK(last_step < kLimits.velocity_max * 1e-3);
}

// The validator refuses a plan the governor would otherwise "fix" by clamping
// mid-motion, and refuses one whose duration means the planner is broken.
void test_trajectory_validator_refuses_bad_plans() {
    const robonode::TrajectoryValidator validator;
    CHECK(validator.check({{0.0, 400.0}}, {kLimits}).ok());
    CHECK(!validator.check({{0.0, 1e6}}, {kLimits}).ok());                 // outside travel
    CHECK(!validator.check({{0.0, std::nan("")}}, {kLimits}).ok());        // non-finite
    CHECK(!validator.check({{0.0}, {0.0}}, {kLimits}).ok());               // axis count mismatch

    // A settling overshoot at the soft limit is measurement noise, not an error.
    CHECK(validator.check({{kLimits.position_max + 1e-4, 400.0}}, {kLimits}).ok());
}

// Jog: an OTG drives one axis while the rest hold, all through one executive.
void test_otg_jog_streams_one_axis_while_others_hold() {
    robonode::SimAxis moving{"a", 0.0, 0.005}, parked{"b", 120.0, 0.005};
    robonode::Governor g0{kLimits}, g1{kLimits};
    robonode::Otg otg{1000.0, kLimits, 0.0};
    otg.retarget(300.0);
    robonode::HoldSource hold{120.0};

    robonode::SyncExecutive exec{{&moving, &parked}, {&g0, &g1}, 1000.0, {}, 0.3};
    std::vector<std::vector<robonode::TelemetryRow>> rows;
    std::vector<robonode::SetpointSource*> sources{&otg, &hold};
    exec.execute(sources, rows, robonode::Otg::estimate_duration(kLimits, 300.0) + 0.2);

    CHECK(std::abs(rows[0].back().actual_position - 300.0) < 1.0);
    CHECK(std::abs(rows[1].back().actual_position - 120.0) < 0.5);
}


// A goal can carry how the target is MOVING. A planner that ignores it reaches
// a point the workpiece has already left; the rendezvous planner meets it
// upstream and runs alongside into the grasp.
void test_rendezvous_planner_approaches_along_the_targets_motion() {
    const robonode::Vec3 target{0.9, 0.25, 0.37};
    const robonode::Vec3 velocity{-0.08, 0.0, 0.0};

    robonode::Goal moving;
    moving.kind = robonode::Goal::kCartesianPosition;
    moving.cartesian = target;
    moving.velocity = velocity;
    CHECK(moving.is_moving());

    robonode::Goal still = moving;
    still.velocity = {};
    CHECK(!still.is_moving());
}

}  // namespace

// A plan that cannot be made says why, on the way back, instead of throwing
// through the executive on the thread that owns the physics.
void test_a_bad_plan_is_refused_with_a_reason() {
    const auto ragged = robonode::SyncBlendPlan::make({{0.0, 1.0, 2.0}, {0.0, 1.0}},
                                                      {kLimits, kLimits});
    CHECK(!ragged.has_value());
    CHECK(ragged.error().message().find("ragged") != std::string::npos);

    const auto single = robonode::SyncBlendPlan::make({{0.0}}, {kLimits});
    CHECK(!single.has_value());
    CHECK(single.error().message().find("two waypoints") != std::string::npos);

    robonode::AxisLimits dead{};
    const auto no_limits = robonode::SyncBlendPlan::make({{0.0, 1.0}}, {dead});
    CHECK(!no_limits.has_value());
    CHECK(no_limits.error().message().find("axis 0") != std::string::npos);

    const auto mismatch = robonode::SyncBlendPlan::make({{0.0, 1.0}}, {kLimits, kLimits});
    CHECK(!mismatch.has_value());
    CHECK(mismatch.error().message().find("limit sets") != std::string::npos);
}

int main() {
    test_a_bad_plan_is_refused_with_a_reason();
    ...
    test_control_smooth_tracks_gentler_than_direct_on_an_ideal_axis();
    ...
    test_rendezvous_planner_approaches_along_the_targets_motion();
    test_cancel_decelerates_on_path_and_reports();
    test_trajectory_validator_refuses_bad_plans();
    test_otg_jog_streams_one_axis_while_others_hold();
