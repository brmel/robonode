...
//
// Identical motion core code that drives SimAxis; only the adapter differs.
// That is the AxisAdapter seam doing its job.
//
//   ./ur_governed_move [robot_ip=127.0.0.1]
//
// Requires URSim powered on with brakes released (dashboard: power on /
// brake release). Writes ur-move.csv + ur-move.mcap.
...
#include "robonode/ur_wrist_adapter.hpp"
...
int main(int argc, char** argv) {
    ...
    const auto plan = robonode::MotionPlan::scurve(
        q5, q5 + 0.3, {wrist_limits.velocity_max_mm_s, wrist_limits.acceleration_max_mm_s2,
    ...
    robonode::McapRecorder::write("ur-move.mcap",
                                  {"rn/dev-cell/ur10e-wrist3/MotionAxis/telemetry"}, {rows},
                                  500.0);
    std::printf("recording: ur-move.mcap\n");
    return 0;
}
