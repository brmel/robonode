...
The physics twin. Turns the `AxisAdapter` seam into real dynamics: a MuJoCo world driven by the same governor/executive/celld that drive `SimAxis`, so following error, servo tracking, and blend behaviour become physical instead of a first-order filter.
...
- One world may back many joints (an arm). Exactly one adapter is the `clock_owner` and calls `mj_step`; `SyncExecutive` phases write→step→read so every setpoint is in place before that single step. `MujocoWorld::step` accumulates a residual so sim time tracks the control clock whatever the timestep/rate ratio.

## Worlds

- `rail.xml` — single linear axis, gravity real; the 65 mm physical following error comes from inertia.
- `rail_ur10e.xml` — rail + UR10e-parameterised 6R arm, 7 DOF sharing one world. UR10e link lengths (`d1,a2,a3,d4,d5,d6`), primitive capsule geometry, gravity-compensated links. **Contacts are on and grouped by role** (ADR-16): the arm is stopped by structure and by parts, self-collision stays off so the control twin never holds its own joints off target, the belt carries parts and ignores the arm, and the suction tip may reach into the part it welds to. Collision-aware *planning* is still the cell reports contacts, it does not yet avoid them. **Servo gains are nominal** — the platform drives every commanded (governed) setpoint to the exact target on one clock; the joints' physical tracking to within ~0.1 rad is a realistic following error, not a defect. Tightening it is inner-loop control tuning, orthogonal to coordination, and cross-checks kinematics against MuJoCo's FK regardless.

## Arm: shared world

`MujocoWorldPool` shares one `MujocoWorld` across every adapter that names the same MJCF path, so the arm's 7 descriptors (one `world`) drive a single physics body; the first node is the clock owner (steps physics). celld keeps its per-node model and stays vendor-blind. `tests/arm_tests` proves one-clock + governed-exact + a protective stop on one joint holding all 7 (cell-coherent safety).
