...
// Returns the pool so a caller can reach the SAME live world the adapters
// drive (for the Scene seam): a second load would be a different universe.
inline std::shared_ptr<MujocoWorldPool> register_mujoco_axis(DriverRegistry& registry) {
