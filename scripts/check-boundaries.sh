...
# sandbox: the untrusted-code engine depends on nothing but core
check sandbox 'robonode/(motion|recorder|adapter|sim_mujoco|celld|gateway|vision)|trajlib/|ruckig/|mcap/|mujoco/|httplib|ur_client_library/' \
  "sandbox may include core only"
...
hits=$(grep -rnE '^#include [<"]httplib' core motion recorder celld engines/sim-mujoco adapters 2>/dev/null || true)
if [ -n "$hits" ]; then
  violation "httplib is private to gateway/ + its server app"
  echo "$hits" | sed 's/^/    /'
fi

# gateway: a library must not choose vendors. Composition roots (apps/) do that
# through robonode::app_support, so the gateway stays hardware-free.
check gateway 'robonode/adapter' \
  "gateway must not name a vendor adapter (apps choose vendors via robonode::app_support)"

# trajlib and ruckig are motion-private
hits=$(grep -rnE '^#include ["<](trajlib|ruckig)/' apps tests adapters core recorder celld engines/sim-mujoco 2>/dev/null || true)
if [ -n "$hits" ]; then
...
hits=$(grep -rnE '^#include [<"]robonode/rtb/' core motion recorder celld engines/sim-mujoco adapters gateway 2>/dev/null || true)
if [ -n "$hits" ]; then
