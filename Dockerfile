...
# OpenCV is the real vision node behind the `Detector` seam — the shipped
# moving-target application selects it, so a container without it would ship a
# platform whose flagship scenario cannot run.
RUN apt-get update && apt-get install -y --no-install-recommends \
      build-essential cmake git ca-certificates libopencv-dev \
...
RUN apt-get update && apt-get install -y --no-install-recommends \
      libstdc++6 ca-certificates curl libopencv-core406t64 libopencv-imgproc406t64 \
    && rm -rf /var/lib/apt/lists/*
# Same path as the build stage so the binary's RPATH to libmujoco stays valid.
COPY --from=build /src /src
ENV ROBONODE_WORLDS_DIR=/src/engines/sim-mujoco/worlds \
