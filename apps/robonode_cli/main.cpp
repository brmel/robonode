...
int main(int argc, char** argv) {
    ...
    if (*c_nodes) {
        ...
        // Deploy the app: the gateway runs its whole program on the worker
        // (#61/#64). Wait for the run to start and finish (running true→false).
        if (const auto st = p.run_app(appfile); !st.ok()) return fail(st);
        bool started = false;
        wait_until([&] { return p.telemetry_json(); },
                   [&](const nlohmann::json& j) {
                       const bool running = j.value("running", false);
                       started |= running;
