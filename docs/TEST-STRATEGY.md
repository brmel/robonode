...
| **CLI e2e** | Same journeys headless, `--json` — agent parity with the UI | `robonode` CLI + robonode_cli_nodes ctest | `mujoco` CI |
...
| J3 | Per-node driver swap, live (try each version; incl. clock-owner swap ) | | | | ** live** | `test_gateway_swaps_one_node_driver_live`; `test_gateway_swap_clock_owner_keeps_physics_alive`; smoke "trajectory capability swap (moveJ)" |
...
| J18 | User authors a sandboxed vision algorithm — compile → run → validate | | | | **** | `test_gateway_define_user_vision_module`; smoke "vision authoring (sandbox compile + register)" |
| J19 | Web algorithm editor — write → compile → register a new version live | | | — | **** | `test_gateway_define_user_vision_module`; `cell.spec.ts` "freshly authored module" |
| J20 | User trajectory code — sandboxed approach planner (via+end), reaches goal | | | | **** | smoke "user trajectory code plans the move" |
| J21 | User modules persist — define, restart, reload + re-register on boot | | — | | **** | smoke "a user module outlives the process that authored it" |
