...
- **No hand-rolled kinematics.** `bridges/rtb` implements the motion `Kinematics`/`Planner` interfaces against the service; the RT streaming loop stays pure C++.
- **Verified:** `rtb_dev` plans a real UR10 Cartesian moveL via the service — 6 joints × N real IK waypoints, FK lands on target to 1e-6 m. `services/rtb-kinematics/test_service.py` self-checks the library; `scripts/rtb-verify.sh` runs the full C++↔service loop.
