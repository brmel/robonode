...
**Consequences:** new dependency `pinocchio` owned by a C++ kinematics impl behind the seam (issue #34, P0); `engines/rtb` + `services/rtb-kinematics` stay for offline use and the real-robot model catalogue; analytic derivatives unlock gradient-based Tier-C planning (Crocoddyl, #41). The `UsingRealRobot` thesis is unchanged — RTB still supplies validated real-robot models; Pinocchio just does the math the loop needs.
...
**Why:** these numbers are the ones an operator most wants to change and most often gets wrong, and every one of them was a literal scattered through the code that used it. A heuristic you cannot change without a rebuild is a heuristic nobody will improve. This is the anti-hardcoding rule (#32) applied to behaviour, not just to identifiers and paths.
