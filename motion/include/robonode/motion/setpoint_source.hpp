...
// Park an axis at a fixed position while other axes stream.
class HoldSource final : public SetpointSource {
public:
    explicit HoldSource(double position) : position_{position} {}

    State next(double, double) noexcept override { return {position_, 0.0, 0.0}; }
    [[nodiscard]] double duration_s() const noexcept override { return 0.0; }
