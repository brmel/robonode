...
public:
    ...
    // A plan, or why these waypoints cannot be one. The RT layer must not throw
    // across a seam — an exception here would unwind through the executive on
    // the thread that owns the physics, and the caller who passed a ragged list
    // is entitled to a sentence rather than a terminate().
    static Result<SyncBlendPlan> make(const std::vector<std::vector<double>>& waypoints,
        ...
        if (n_axes == 0 || limits.size() != n_axes) {
            return no("plan: " + std::to_string(n_axes) + " axes but " +
                      std::to_string(limits.size()) + " limit sets");
        }
        const std::size_t m = waypoints[0].size();
        if (m < 2) return no("plan: a motion needs at least two waypoints");
        for (const auto& w : waypoints) {
            if (w.size() != m) {
                return no("plan: ragged waypoints — one axis has " + std::to_string(w.size()) +
                          ", another " + std::to_string(m));
            }
        }
        for (std::size_t i = 0; i < limits.size(); ++i) {
            if (limits[i].velocity_max <= 0.0 || limits[i].acceleration_max <= 0.0) {
                return no("plan: axis " + std::to_string(i) + " has no usable velocity or "
                          "acceleration limit");
            }
        }
