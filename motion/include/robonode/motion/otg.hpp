...
public:
    ...
    // A conservative time bound for a rest-to-rest move, from the limits alone.
    // duration_s() only becomes meaningful after the first update, so a caller
    // that must size a run window up front asks for this instead.
    [[nodiscard]] static double estimate_duration(const AxisLimits& limits, double distance) {
        const double d = std::abs(distance);
        if (limits.velocity_max <= 0.0) return 0.0;
        double t = d / limits.velocity_max;
        if (limits.acceleration_max > 0.0) t += 2.0 * limits.velocity_max / limits.acceleration_max;
        if (limits.jerk_max > 0.0) t += 2.0 * limits.acceleration_max / limits.jerk_max;
        return t;
    }
